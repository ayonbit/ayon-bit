...
// Safe extraction of the first <p> or <h*> tag using regex (SSR compatible)
const extractFirstParagraphOrHeading = (html) => {
  if (!html) return "";
  const match = html.match(/<(p|h[1-6])[^>]*>(.*?)<\/\1>/i);
        ...
        <div className="flex flex-col flex-grow p-4 sm:p-5">
          ...
          <div className="flex items-center justify-between gap-3 border-t border-accent pt-3 sm:pt-4 mt-auto">
            ...
            <div className="flex flex-col items-center gap-2">
              ...
              <BsArrowDownRight className="text-gray-400 group-hover:text-accent transition-colors" />
            </div>
          </div>
        </div>
      </Link>
